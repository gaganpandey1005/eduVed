  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        ...
        setBook(res.data);
        sessionStorage.setItem("book", JSON.stringify(res.data.book));
...
const handleBought = async () => {
  try {
    ...
    const bookData = JSON.parse(sessionStorage.getItem("book"));
    const bookSellerId = bookData.soldBy;
    const bookId = bookData._id;

    // Step 1: Get payment order from backend
    const { data } = await apirequest.post(`/shivani/bought`, {
      loggedInUserId,
      bookSellerId,
      bookId,
    });
    ...
    const { orderId, amount, currency } = data.paymentInfo;
    ...
    const options = {
      ...
      currency,
      name: "Book Purchase",
      description: "Complete your book payment",
      order_id: orderId,
      handler: async function (response) {
        
        toast.success("Payment successful!");

        // Step 2: Confirm payment and update DB
       const {data}= await apirequest.post(`/shivani/confirm-payment`, {
          loggedInUserId,
          bookSellerId,
          bookId,
        });
        ...
        const userData=JSON.parse(sessionStorage.getItem("user"))
        userData.bookSolded=data.user.bookSolded;
        userData.buyBooks=data.user.buyBooks;
      ...
      >
        <div className="text-center p-8 bg-gray-800 rounded-xl shadow-2xl">
          ...
          <p className="text-white mb-6">
            Sorry, we couldnt find the book youre looking for.
          </p>
          <button
            onClick={() => navigate(-1)}
                  ...
                  <div className="flex items-center bg-gray-700 bg-opacity-40 p-3 rounded-lg">
                    ...
                    <span
                      className={`font-medium ${
                        !paid ? "blur-sm text-gray-400" : "text-white"
                      }`}
                    >
            ...
            >
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <p className="text-lg mb-2">
                  ...
                  <span className="font-bold">{book.userEmail}</span>
                </p>
                <p className="text-gray-600">
                  You can now contact the seller to arrange pickup or delivery
                  of your book.
                </p>
              </div>
